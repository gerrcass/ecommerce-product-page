...
const HeroText = ({ product, addToCard }) => {
    ...
    return (
        <>
            <h2 className="tracking-wider text-primary-orange uppercase text-sm font-bold mb-4">{company}</h2>
            <h1 className="text-4xl font-extrabold mb-8 text-black">{name}</h1>
            <p className="text-neutral-darkgrayishblue text-sm mb-5 leading-6">{desc}</p>

            <div className="mb-5">
                <div className="flex items-center">
                    <span className="text-2xl font-bold text-black">${(price * (discount_percent / 100)).toFixed(2)}</span>
                    <span className="ml-2 bg-primary-pale text-sm text-primary-orange leading-none py-1 px-2 rounded-lg font-semibold">50%</span>
                </div>
                <span className="text-neutral-grayishblue line-through text-sm font-semibold">${price.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
                <div className={`${quantity_available === 0 && 'opacity-40'} w-32 h-12 flex justify-between items-center rounded-xl bg-neutral-lightgrayishblue`}>
                    ...
                    <span className="text-center text-sm font-semibold select-none w-4">{count}</span>
                ...
                <button
                    ...
                    className="disabled:bg-opacity-40 flex items-center ml-4 h-12 text-base font-bold bg-primary-orange px-16 rounded-lg bg-opacity-90 hover:bg-opacity-100 transition duration-100 ease-in-out active:scale-x-[1.005]" type="button">
