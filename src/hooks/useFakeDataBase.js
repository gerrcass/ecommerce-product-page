...
const useFakeDataBase = () => {
    ...
    cartItems.items.reduce((accumulator, currentValue) => {
      return (
        ...
        currentValue.quantity *
          (currentValue.price * (currentValue.discount_percent / 100))
      );
    }, 0);
  ...
  const addToCard = (productId, quantity) => {
    ...
    const itemIsInCart = cartItems.items.findIndex(
    ...
    const updatedTotal =
      quantity * price * (discount_percent / 100) + getCurrentTotal();
    ...
    if (itemIsInCart === -1) {
      ...
      const updatedItems = cartItems.items.map(
        (item) =>
          (item.id = id
    ...
    setProducts(
      products.map((product) =>
        product.id === productId
          ? { ...product, quantity_available: initialStockExample }
          : product
      )
    );
  };
